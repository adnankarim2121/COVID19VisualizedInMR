...
public class MetalContentHandler : MonoBehaviour
{
    ...
    public bool confirmMetalSurfaceSize = false;
    GameObject textForMetal;
    ...
    void Start()
    {
        confirmButton = GameObject.Find("ConfirmationButtonMetal");
        ...
        textForMetal = GameObject.Find("InfoForMetal");
        description = textForMetal.gameObject.transform.GetChild(1).gameObject;
        ...
        double infectivityRateMetal = formulaForInfectivityRate(tempVal, humidVal);
        textMeshPro.SetText("The infectivity rate for metal is: " + infectivityRateMetal.ToString() + " per min");


        //Survival rate handeling
        //GetComponent<TextMesh>().text = "Adnan";
        survivalInfoMetal = GameObject.Find("SurvivalInfoMetal");
        descriptionSR = survivalInfoMetal.gameObject.transform.GetChild(1).gameObject;
        textMeshProSR = descriptionSR.GetComponent<TextMeshPro>();
        currentTime = DateTime.Now;
        survivalRateInformatioString = "The time that COVID-19 was found on this metal surface was at: " + currentTime.ToString() + "\n"
            + "The survival rate of the virus on this surface at these conditions are: " + calculateSurvivalRateMetal().ToString() + " hours" + "\n"
            + "This surface will be safe to touch at: " + currentTime.AddHours(calculateSurvivalRateMetal()).ToString();
        textMeshProSR.SetText(survivalRateInformatioString);
        ////fast forward button
        fastForwardButton = survivalInfoMetal.gameObject.transform.GetChild(2).gameObject.transform.GetChild(1).gameObject;
        ...
        currentTimeButton = survivalInfoMetal.gameObject.transform.GetChild(2).gameObject.transform.GetChild(2).gameObject;
    ...
    void Update()
    {
        if (confirmMetalSurfaceSize)
        {
            //survivalInfoMetal.SetActive(true);
            //ensure button is clicked only once
            confirmMetalSurfaceSize = false;
        }
    ...
    {
        confirmMetalSurfaceSize = true;
        confirmButton.SetActive(false);
        GameObject.Find("CubeForMetal").GetComponent<BoundsControl>().enabled = false;
    ...
    void updateContent()
    {
        ...
        var cubeRenderer = GameObject.Find("CubeForMetal").GetComponent<Renderer>();
    ...
    public double calculateSurvivalRateMetal()
    {
